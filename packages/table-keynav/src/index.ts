...
export class GridKeyNav {
  ...
  public cellNavigation(e: KeyboardEvent) {
    ...
    if (e.key === Keys.ArrowRight || e.key === Keys.ArrowDown) {
      ...
      // const focusableWidgets = [...cell.children].filter((child) =>
      //   child.matches(this.selectors.Focusable)
      // );
      // console.log('---wowowowow---');
...
const singleton = new GridKeyNav({ debug: true });

export const libraryHooks = {
  onKeyDown: (e: any) => {
    singleton.tableKeyDown(e);
  },
  onKeyUp: () => {
    singleton.tableKeyUp();
  },
};
